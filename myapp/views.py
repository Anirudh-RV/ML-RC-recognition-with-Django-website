...
def runscanner(request):
    ...
    cur = conn.cursor()
    cur.execute(" Create table if not exists HackData(state text, registration_no text,owner text, model text, makers_name text , year_of_manufacture text, chassis_no text,engine_no text,reg_date text, valid_date text, road_tax_upto text, fuel_used text) ")
